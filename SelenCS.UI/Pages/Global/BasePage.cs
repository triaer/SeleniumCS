...
namespace SelenCS.UI.Pages.Global
{
    public abstract class BasePage
    {
        ...
        internal static IWebElement FindElement(By by, int timeout = shortTimeout)
        {
            try
            {
                return WebDriver.Wait(timeout).Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(by));
            }
            catch (Exception)
            {
                return null;
        ...
        {
            var wait = WebDriver.Wait(seconds);
        ...
        internal static void WaitForElement(By elementDescription, int timeout = longTimeout)
        {
        ...
        {
            IWebElement myDynamicElement = (WebDriver.Wait(seconds)).Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(elementDescription));
        }
        ...
        {
            var wait = WebDriver.Wait(timeoutSec);
            wait.Until((driver) =>
            {
                try
                {
        ...
        {
            var wait = WebDriver.Wait(timeout * 2);
        ...
        internal static void WaitForJQueryToBeActive(int timeout = longTimeout)
        {
            ...
            int numOfWait = 1;
            bool isJqueryUsed = (bool)(WebDriver.ExecuteScript("return (typeof(jQuery) != 'undefined')"));
            if (isJqueryUsed)
            {
                while (true)
                {
                    // JavaScript test to verify jQuery is active or not
                    bool ajaxIsComplete = (bool)(WebDriver.ExecuteScript("return jQuery.active == 0"));
        ...
        internal static void WaitForAngularJSLoad(int timeout = mediumTimeout)
        {
            ...
            {
                isDataLoaded = (bool)WebDriver.ExecuteScript("return (window.angular != undefined) && (angular.element(document).injector() !== undefined) && (angular.element(document).injector().get('$http').pendingRequests.length === 0)");
                Wait(1);
                numOfWait++;
        ...
        internal static void SwitchToNewPopUpWindow(IWebElement ElementToBeClicked, out string parentWindow, bool closePreviousWindow = false, bool doubleClick = false, int timeout = 30)
        {
            ...
            try
            {
                ...
                string popUpWindowHandle = WebDriver.Wait(timeout).Until<string>((d) =>
                {
        ...
        public T LogValidation<T>(ref List<KeyValuePair<string, bool>> validations, List<KeyValuePair<string, bool>> addedValidations) where T : BasePage
        {
